...
export class AuthController {
    ...
    static signUpUser: RequestHandler = asyncHandler(async (req: Request<{}, {}, IRegisterReq>, res: Response) => {
        ...
        if (existingUser) throw new CustomError('User already exists please login', 400)
    ...
    static verifyEmail: RequestHandler = asyncHandler(async (req: Request<{}, {}, IOtpReq>, res: Response) => {
        ...
        if (!email || !otp) throw new CustomError('All fields are required', 400)

        const existingUser: IUser | null = await UserModel.findOne({ email })
        if (!existingUser) throw new CustomError('User not found', 404)
        if (existingUser.is_varified) throw new CustomError('User already varified', 400)
        ...
        // send response
        res.status(200).json(new ApiResponse(200, 'Email verified successfully, Now you can login', {}))
    });
    ...
    static refreshToken: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
        ...
        // verify token
        const decoded: IPayload = await UtilityFunctions.verifyToken(refreshToken, process.env.JWT_REFRESH_TOKEN_SECRET!)
    ...
    static resetPassword: RequestHandler = asyncHandler(async (req: Request<IResetParams, {}, IChangePasswordReq>, res: Response) => {
        ...
        // update password
        const updatedUser: IUser | null = await UserModel.findByIdAndUpdate(id, { password: hashedPassword })
